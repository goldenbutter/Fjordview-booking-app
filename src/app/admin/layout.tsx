      ...
      <div className="min-w-0">
        <header className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-white px-5 py-4 print:hidden">
          <div>
            ...
            <div className="font-semibold">{context?.adminUser?.name ?? "Demo Admin"}</div>
          </div>
          <div className="flex items-center gap-3 text-sm">
            {property ? (
              <Link href={`/book/${property.slug}`} className="rounded-md border border-teal-200 px-3 py-2 font-semibold text-teal-700">
                Public booking
              </Link>
            ) : null}
